# flatmate_ai_v3.py — FlatMate AI (Summarizer + Task Tracker + Reminders)

from transformers import pipeline
from datetime import datetime
from config import HUGGINGFACE_TOKEN, DATA_DIR
import os, re, json, sys, statistics, schedule, time


# ----------------------------------------------------------------------
# 1. Model initialization (load once for efficiency)
# ----------------------------------------------------------------------
...
def extract_tasks_from_chat(messages):
    """
    Extracts 'who will/can/to <verb>' directly from chat messages.
    """
    verbs = [
        "pay", "buy", "clean", "cook", "fix", "call", "collect",
        "arrange", "handle", "book", "water", "organize",
        "manage", "repair", "wash", "prepare"
    ]
    task_list = []
    for msg in messages:
...
def analyze_conversation():
    ...
    sentiment_results = sentiment_analyzer(conversation)
    labels = [r["label"] for r in sentiment_results]
    pos = labels.count("POSITIVE")
    neg = labels.count("NEGATIVE")
    mood = "Positive" if pos >= neg else "Negative"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--done":
        if len(sys.argv) < 4:
            print("Usage: python flatmate_ai_v3.py --done <Person> <Keyword>")
        else:
